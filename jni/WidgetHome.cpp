...
bool WidgetHome::onTouch(View* vView, MotionEvent* ev)
{
    ...
    if (w->mTouchState == 1 && vView == mCategoryId) {
        ...
        w->mEventAction = [](Wrapper* w)->void{
            w->mNActivity->sendOp(w->mOpUnitUIId, w->w->mNEta00, new OpMessage("sCategoryId", "", "", "" , "", w->mEventString0 == "sCategoryId" ? !w->mEventLong1 : w->mEventLong1 ));
        };
    }
    if (w->mTouchState == 1 && vView == mTitle) {
        ...
        w->mEventAction = [](Wrapper* w)->void{
            w->mNActivity->sendOp(w->mOpUnitUIId, w->w->mNEta00, new OpMessage("sTitle", "", "", "" , "", w->mEventString0 == "sTitle" ? !w->mEventLong1 : w->mEventLong1 ));
        };
    }
    if (w->mTouchState == 1 && vView == mIn) {
        ...
        w->mEventAction = [](Wrapper* w)->void{
            w->mNActivity->sendOp(w->mOpUnitUIId, w->w->mNEta00, new OpMessage("sIn", "", "", "" , "", w->mEventString0 == "sIn" ? !w->mEventLong1 : w->mEventLong1 ));
        };
    }
    if (w->mTouchState == 3) {
        w->mTouchState = 0;
