...
public class TKrossSystem extends TKrossVisitor
{
    ...
    public TKrossSystem(TWrapper w)
    {
        super(w);
        ...
        w.aTVisitor = new TKrossVisitor[] {
            ...
            w.mTVisitorHardware = new TVisitorHardware(w),
            w.mTVisitorIO = new TVisitorIO(w),
            w.mTVisitorLocation = new TVisitorLocation(w),
            w.mTVisitorMicroedition = new TVisitorMicroedition(w),
            w.mTVisitorOpengl = new TVisitorOpengl(w),
    ...
    public void tInit()
    {
        ...
        putNext(w.tActivity.getResources().getAssets());

        // Get native wrapper, visitors, elements, and activity
        w.mTKrossSystem.n = nInit(0);
        w.sTVisitor.put(w.mTKrossSystem.n, this);

        w.mTAlpha00.n = nInit(w.mTKrossSystem.n);
    ...
    public Object tRunObject(long a, long b)
    {
        ...
        if (b == -2) {
            ...
            if (b >= 0 && ret instanceof List<?>) {
                size = ((List<?>)ret).size();
                ret = ((List<?>)ret).get((int)b);
            }
            if (b >= size - 1) {
                synchronized (w.aObject) {
    ...
    public long visit(TAlpha01 element, long a, long b, long c, long d, long e)
    {
        ...
        if (o instanceof List<?>) {
            ...
            if (b >= 0 && c == 0) {
                ret = ((Number)((List<?>)o).get((int)b)).longValue();
            }
            if (c != 0) {
                synchronized (w.sObject) {
                   ret = emplaceKey(c, ((List<?>)o).get((int)b));
                }
            }
        }
        if (b >= size - 1) {
    ...
    public native long nCancel(long vNKrossSystem);
    public native long nInit(long vNKrossSystem);
