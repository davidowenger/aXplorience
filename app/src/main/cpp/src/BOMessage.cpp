...
void BOHandlerMessage::addDrop(OpUnitUI* vOpUnitUI, String packed, String vSourceMac)
{
    ...
    if (vSize >= 5) {
        ...
        if (vDBCollection->count()) {
            ...
            if (!vExisting->is("sIn") && !vExisting->is("sArchivedUser") && vExisting->get("sRevBuzz") == aValue[3] && vExisting->get("sRevRemote") != aValue[4]) {
                ...
                ++w->mcInterrupt;
                vOpUnitUI->sendOp(w->mOpUnitAnimId, w->mNWrapper->mNKrossWrapper->mNAlpha00, new OpParam(vExisting->mId));
                vExisting->set("sRevBuzz", vExisting->count("sRevBuzz") + 1);
                vExisting->set("sRevRemote", "1");
                w->maMessageBuzz->add(new String("2#" + packId(vExisting) + "3#"));
                vExisting->set("sBuzzed", kTrue);
                vOpUnitUI->sendOp(w->mOpUnitAppId, w->mNWrapper->mNKrossWrapper->mNEta01, new OpParam((NParam)get(vExisting->mId)));
            }
            if (vExisting->is("sIn") && vExisting->count("sRevSource") < to_long(aValue[2])) {
                ...
                if (vSize > 5) {
                    ...
                    vReceived->drop();
                    vOpUnitUI->sendOp(w->mOpUnitAppId, w->mNWrapper->mNKrossWrapper->mNGamma01, new OpParam((NParam)getMessagesToDisplay()->sort(w->maSort)));
                    delete vReceived;
                }
