...
void OpUnitCore::run()
{
    ...
    if (vaApplication->count()) {
        ...
        w->mDBObjectApplication->set("sPseudoMac", UUID::randomUUID()->toString().substr(20, 12));
    }
    if (vaMessage->count()) {
    ...
    w->mcConnected = 0;
    w->mac = w->mDBObjectApplication->get("sPseudoMac");
    w->sServiceName = "Proximity service";
    w->sUuidService = "0000F9B3-0000-0000-0000-020000000000";
