...
void OpUnitUI::addDrop(String& vPacked, String& vSourceMac)
{
    nint vcStart;
    nint vcEnd;
    ...
    while (mAlive && (vcStart = vPacked.find("2#")) >= 0 && (vcEnd = vPacked.find("3#")) >= 0 && vcStart + 2 < vcEnd) {
        w->mBOHandlerMessage->addDrop(this, vPacked.substr(vcStart + 2, vcEnd - (vcStart + 2)), vSourceMac);
...
void OpUnitUI::buzz(nuint vcDBObjectId, bool vIsBuzzed)
{
    ...
    if (vIsBuzzed && vDBObject->is("sEnabled") && !vDBObject->is("sBuzzed")) {
        ...
        sendOp(w->mOpUnitAnimId, w->mNWrapper->mNKrossWrapper->mNAlpha00, new OpParam((NParam)vcDBObjectId));
        vDBObject = nullptr;
    }
    if (!vIsBuzzed) {
...
{
    addDrop(((OpMessage*)a)->mStringA, ((OpMessage*)a)->mStringB);
    return 0;
}
